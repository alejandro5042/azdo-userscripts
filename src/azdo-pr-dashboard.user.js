...
(function() {
    ...
    const onPageUpdatedThrottled = _.throttle(onPageUpdated, 400, {
        leading: false,
        trailing: true
    });
        ...
        $('.vc-pullrequest-leftpane-section.files-tab').once('annotate-with-owners-info').each(async() => {
            ...
            annotateFilesTreeFunc = function() {
                ...
                $('.vc-sparse-files-tree .vc-tree-cell').once('annotate-with-owners-info').each(function() {
                    ...
                    const {
                        fullName: pathWithLeadingSlash,
                        isFolder,
                        depth
                    } = getPropertyThatStartsWith(listItem, '__reactEventHandlers$').children.props.item;
        ...
        .base-selector select option:disabled {
          display: none;
            ...
            selector.children('option').each(function() {
                $(this).text((_i, text) => text.replace(/ /g, '\xa0'));
            });
            ...
            selector.on('change', function(_event) {
    ...
    function addNICodeOfDayToggle() {
        ...
        $('.vc-discussion-comment-toolbar').once('add-cod-flag-support').each(async function() {
            ...
            button.click(async function(_event) {
    ...
    function highlightDiff(language, _fileName, _part, diffContainer, selector) {
